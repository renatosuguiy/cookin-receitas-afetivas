context('Login', () => {
    it('Enters landing page, tries to go to the Login page and check Myrecipes page', () => {
        ...
        cy.contains("Bolo de Maçã Vegano");

        cy.get("button[data-index=1]").click();

        cy.contains("Você ainda não tem receitas cadastradas.");

        cy.get("button[data-index=2]").click();

        cy.contains("Você ainda não tem receitas favoritas.");
    })
})